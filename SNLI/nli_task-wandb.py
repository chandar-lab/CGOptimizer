...
def evaluate(nli_net, valid_iter, optimizer, epoch, train_config, params, eval_type='valid', test_folder = None, inv_label = None, itos_vocab = None, final_eval=False):
    nli_net.eval()
    correct = 0.
    test_prediction = []
    s1 = []
...
#    torch.manual_seed(params.seed)
    ...
    DATA
    """
    train, valid, test, vocab, label_vocab = DataIteratorGlove(batch_size = params.batch_size, dataset = params.nlipath ,max_length = 20, prefix='processed_')
    ...
    while not train_config['stop_training'] and epoch <= params.n_epochs:
        ...
        eval_acc, optimizer, train_config = evaluate(nli_net, valid, optimizer, epoch, train_config, params, eval_type='valid')
        #scheduler.step()
        ...
        wandb.log({"Train Accuracy": train_acc, "Val. Accuracy": eval_acc, "offline updates" : off, "online udpates": on})
