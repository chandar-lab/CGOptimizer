...
class Ackley(OptimalBasic):
    ...
    def get_func_val(self, variables):
        ...
        tmp1 = torch.tensor(20.-20.*torch.exp(-0.2*torch.sqrt(1./self.variable_num*torch.sum(torch.FloatTensor([v*v for v in variables])))))
        pi = torch.acos(torch.zeros(1))
        tmp2 = torch.tensor(torch.exp(torch.tensor(1.0))-torch.exp(1./self.variable_num*torch.sum(torch.FloatTensor([torch.cos(torch.tensor(v*2.0*pi)) for v in variables]))))
        return tmp1+tmp2
...
class GoldsteinPrice(OptimalBasic):
    ...
    def get_func_val(self, variables):
        tmp1 = (1+torch.pow(variables[0]+variables[1]+1,2)*(19-14*variables[0]+3*torch.pow(variables[0],2)-14*variables[1]+6*variables[0]*variables[1]+3*torch.pow(variables[1],2)))
        tmp2 = (30+(torch.pow(2*variables[0]-3*variables[1],2)*(18-32*variables[0]+12*torch.pow(variables[0],2)+48*variables[1]-36*variables[0]*variables[1]+27*torch.pow(variables[1],2))))
        return tmp1*tmp2
...
class SixHumpCamel(OptimalBasic):
    ...
    def get_func_val(self, variables):
        return 4-2.1*torch.pow(variables[0],2)+1/3*torch.pow(variables[0],4)*torch.pow(variables[0],2)+variables[0]*variables[1]+4*(torch.pow(variables[1],2)-1)*torch.pow(variables[1],2)
